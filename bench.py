...
def iaareaopen(f,a,Bc=iasecross()):
    ...
    #flatten = f.image_rgb.flatten(order='A')
    print(f)
    #s =f.oclPtr.shape
    print("IPL",len(f.ipl[1]))
    s =f.oclPtr.shape
    ...
    fravel = np.zeros(size,dtype =int)
    f1 = np.concatenate((f.flatten(),np.array([0])))
    print("RAVEL",fravel)
    print("TAMANHO RAVEL",len(fravel))
    
    print("F1=",f1)
    print("NUMEROS DE F1=",len(f1))
    
    area = -np.ones((size+1,), np.int32)
    print("Area=",area)
